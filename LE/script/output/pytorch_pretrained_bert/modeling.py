...
class BertForCloth(PreTrainedBertModel):
    ...
    def forward(self, inp, tgt):
        ...
        out, _ = self.bert(articles, attention_mask = articles_mask,
            output_all_encoded_layers=False)  # [batch_size , seq_length , hidden_size]
        question_pos = question_pos.unsqueeze(-1)
        question_pos = question_pos.expand(bsz, opnum, out.size(-1))
        out = torch.gather(out, 1, question_pos) # [batch_size , opnum , hidden_size]
        out = self.cls(out)  # [batch_size , opnum , vocab_size] 30522
        #convert ops to one hot
        out = out.view(bsz, opnum, 1, self.vocab_size) # [batch_size , opnum ,1 , vocab_size] 30522
        out = out.expand(bsz, opnum, 4, self.vocab_size)  # [batch_size , opnum , 4 , vocab_size] 30522 
        out = torch.gather(out, 3, ops) # [batch_size , opnum , 4 , max_option_length] #### ops size(batch_size , opnum , 4 , max_option_length=4)
        #mask average pooling
        out = out * ops_mask
        out = out.sum(-1) # [batch_size , opnum , 4 ]
        out = out/(ops_mask.sum(-1))
        
        out = out.view(-1, 4) #  # [batch_size*opnum , 4 ]
        tgt = tgt.view(-1,) # [batch_size , opnum]  -> # [batch_size*opnum]
        ...
        return out, loss, acc, acc_high, acc_middle
...
if __name__ == '__main__':
    ...
    tgt = ans
    model = BertForCloth.from_pretrained('../bert-base-uncased.tar.gz',
